...
{
    public class LogArchiveOMSService
    {
        ...
        public dynamic LogArchive(ListmodelLogArchive_SearchDate Search)
        {
            try
            {
                var result = new List<ListmodelLogArchiveOMS>();
                var db = new MasterArOMSDbContext();
                var OMSArchiveData = db.sy_LogArchiveData.AsQueryable();
                ...
                if (!string.IsNullOrEmpty(Search.Start_Date) && !string.IsNullOrEmpty(Search.End_Date))
                {
                    ...
                    OMSArchiveData = OMSArchiveData.Where(c => c.ArchiveData_Start_Date >= startDate && c.ArchiveData_End_Date <= endDate); 
                    //return new { message = startDate, endDate};
                    // Map the filtered records to the result
                    if (OMSArchiveData.Any())
                    {


                        foreach (var ar in OMSArchiveData)
                        {
                            var resultItem = new ListmodelLogArchiveOMS();
